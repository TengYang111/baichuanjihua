...
class JinrongjieSpider(baseSpider):
    name = 'xiaofeibaoguang'
    allowed_domains = ['http://finance.jrj.com/']

    def start_requests(self):#还有种是按照月份来构造的，这里只爬取前十页
        bash_url = 'http://finance.jrj.com.cn/consumer/'
        last_url = '.shtml'
        for i in range(1,11):
            if i == 1:
                url = bash_url
            else:
                url = bash_url + 'consumer_index-' + str(i) + last_url
    ...
    def parse(self, response):  # 每个页面所有新闻列表链接
        news_urls = response.xpath('//ul[@class="dotlist1 dashed1"]/li/label/a/@href').extract()
